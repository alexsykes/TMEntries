<x-main>
    ...
    <div class=" bg-white border-1 border-gray-400 rounded-xl  outline outline-1 -outline-offset-1 drop-shadow-lg outline-gray-300 pb-2">
        ...
        <div class="font-bold w-full pt-2 pb-2 pl-4 pr-4 rounded-t-xl  text-white bg-blue-600">Coming up…</div>
            <table class="w-full sm:ml-2">
                @foreach($trials as $trial)
                    <tr class=" flex-auto">
                        <td class="text-sm hidden md:table-cell">{{$trial->date}}</td>
                        <td class="text-sm hidden md:table-cell">{{$trial->club}}</td>
                        <td class="text-sm pl-2 table-cell">{{$trial->name}}</td>
