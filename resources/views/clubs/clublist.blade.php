    ...
    @foreach($clubs as $club)
        <div class=" bg-white border-1 border-gray-400 rounded-xl  outline outline-1 -outline-offset-1 drop-shadow-lg outline-gray-300 pb-2">
            ...
            <div class="text-sm pl-2 pr-2">
                <div class="font-semibold text-blue-800">@php echo $club->description; @endphp </div>
                @if($club->website)
                ...
                @if($club->series)
                    @foreach($club->series as $series)
                        <div class="pt-2 font-semibold">{{$series->name}} trials - {{$series->description}}</div>
                        <div class="">@php echo $series->notes; @endphp</div>

                        <div class="flex justify-between text-blue-800 font-bold w-full  ">
